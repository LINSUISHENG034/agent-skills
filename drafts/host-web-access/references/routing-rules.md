...
- `open-host-page.sh` always returns machine-readable JSON with `route`, `reason`, `needs_browser`, and `origin`; browser routes also report `runtime_status`, `page_status`, `recovery_attempted`, and (when resolved) `target_id`
...
- Assisted recovery includes `assisted_session` plus `lan_novnc_url`; `open-host-page.sh` returns this handoff payload and does not auto-run assisted `capture`/`stop` for blocked states
