...
- `open-host-page.sh` uses assisted mode as a handoff: on `challenge`, `login-wall`, or unrecovered `target-mismatch`, it returns `assisted_session` + `lan_novnc_url` and does not auto-capture before operator takeover
