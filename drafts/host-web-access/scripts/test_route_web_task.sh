...
run_expect() {
  ...
  local output
  output="$("$CMD" "$@")"
  python3 - "$output" "$expected_route" "$expected_reason" <<'PY'
import json
import sys

data = json.loads(sys.argv[1])
assert data.get("route") == sys.argv[2], data
assert data.get("reason") == sys.argv[3], data
assert isinstance(data.get("needs_browser"), bool), data
PY
}
...
run_expect browser login-required --login-required
