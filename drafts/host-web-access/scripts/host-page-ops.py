...
CDP_EVAL_PATH = ROOT_DIR / "ubuntu-browser-session" / "scripts" / "cdp-eval.py"


def load_cdp_module(path: Path):
    spec = importlib.util.spec_from_file_location("cdp_eval", str(path))
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to locate cdp-eval module")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


cdp = load_cdp_module(CDP_EVAL_PATH)
...
def main() -> int:
    ...
    if args.click_link_text:
        result = cdp.click_link(port, target_id, args.click_link_text)
        print(json.dumps(result, ensure_ascii=False))
        return 0
